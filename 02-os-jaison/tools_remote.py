...
def search_pexels_broll(args: Dict[str, Any], **kwargs) -> List[Dict[str, Any]]:
    """
    Search Pexels for free stock videos (b-roll).
    """
    ...
    if not api_key:
        return [{"error": "PEXELS_API_KEY is not set."}]
...
def register_tools(ctx: Any) -> None:
    ...
    ctx.register_tool(
        ...
        schema={
            "name": "video_editor_search_broll",
            "description": "Search Pexels for free stock videos. Provide query and optional orientation ('portrait', 'landscape', 'square').",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {"type": "string", "description": "Search query for b-roll footage."},
                    "orientation": {"type": "string", "description": "Video orientation ('portrait', 'landscape', 'square').", "enum": ["portrait", "landscape", "square"], "default": "portrait"},
                },
                "required": ["query"]
            }
        },
        handler=search_pexels_broll,
    )
    ctx.register_tool(
