...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() => {
    const hasToken = typeof window !== "undefined" && (window.__HERMES_SESSION_TOKEN__ || import.meta.env.DEV);
    return !hasToken
  ...
  useEffect(() => {
    ...
    const token = window.__HERMES_SESSION_TOKEN__ || (import.meta.env.DEV ? "dev-token" : undefined);
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          {/* Agentic OS Header for ChatPane */}
          <div className="flex flex-col px-6 pt-5 pb-2 shrink-0 border-b border-current/10">
            <div className="flex items-center justify-between mb-4">
              <span className="font-mondwest text-xs tracking-widest text-text-tertiary uppercase">
                // -- AGENT / CLAUDE
              </span>
              <Button size="sm" ghost className="h-6 rounded-full border border-current/20 px-3 text-[10px] uppercase font-mondwest tracking-widest text-text-secondary hover:text-midground">
                Command palette ⌘K
              </Button>
            </div>
            
            <h1 className="text-4xl sm:text-5xl font-bold tracking-tight text-midground mb-1">
              Claude
            </h1>
            <p className="text-sm text-text-secondary mb-6">
              Direct streaming channel to your Claude Code CLI...
            </p>

            <div className="flex items-center gap-6">
              <button className="text-sm font-bold text-midground border-b-2 border-midground pb-2 px-1">
                Chat
              </button>
              <button className="text-sm font-medium text-text-secondary hover:text-midground pb-2 px-1 transition-colors">
                Workspace
              </button>
              <button className="text-sm font-medium text-text-secondary hover:text-midground pb-2 px-1 transition-colors">
                Ultracode
          ...
          <div
            ref={hostRef}
            className="hermes-chat-xterm-host min-h-0 min-w-0 flex-1 p-2 sm:p-3"
          />
