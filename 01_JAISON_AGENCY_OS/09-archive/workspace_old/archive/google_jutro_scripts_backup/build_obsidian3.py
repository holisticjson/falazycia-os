...
for week in [1, 2, 3]:
    ...
    modules_str = content[start_idx:end_idx]
    
    mod_parts = modules_str.split("title:'")
    for module_idx, part in enumerate(mod_parts[1:], 1):
        title = part.split("'", 1)[0]
        desc_part = part.split("desc:'")[1] if "desc:'" in part else ""
        desc = desc_part.split("'", 1)[0] if desc_part else ""
        ...
        # lessons
        if "lessons:[" in part:
            lessons_str = part.split("lessons:[")[1].split("]")[0]
            for les_idx, l in enumerate(lessons_str.split("{t:'")[1:], 1):
                les_t = l.split("'", 1)[0]
                les_url = ""
                if "url:'" in l:
                    les_url = l.split("url:'")[1].split("'")[0]
                vid_id = ""
                if 'v=' in les_url:
        ...
        # pdfs
        if "pdfs:[" in part:
            pdfs_str = part.split("pdfs:[")[1].split("]")[0]
            for p in pdfs_str.split("'")[1::2]:  # every odd item is inside quotes
                if p.strip() and len(p) > 2:
...
with open(os.path.join(ROOT_DIR, "Obsidian_Knowledge_Base", "Lista_Materialow_PDF.md"), "w", encoding='utf-8') as f:
