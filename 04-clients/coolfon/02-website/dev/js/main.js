...
document.addEventListener("DOMContentLoaded", () => {
    const initSafe = (name, fn) => {
        try {
            ...
            console.error(`[Coolfon Init Error] Failed to initialize ${name}:`, e);
        }
    };

    initSafe("initNavigation", initNavigation);
    initSafe("initCookieBanner", initCookieBanner);
    initSafe("initRepairCalculator", initRepairCalculator);
    initSafe("init3DTilt", init3DTilt);
    initSafe("initMouseGlow", initMouseGlow);
    initSafe("initScrollReveal", initScrollReveal);
    initSafe("initParticlesCanvas", initParticlesCanvas);
    initSafe("initChatbot", initChatbot);
...
function initChatbot() {
    ...
    // Przechowuje lokalną historię konwersacji na potrzeby pamięci kontekstowej Gemini API
    const chatHistory = [];
    ...
    function handleSendMessage() {
        ...
        .then(data => {
            ...
            if (data.status === "success" && data.reply) {
                // Dodaj odpowiedź asystenta do okna czatu i do historii
                appendBotMessage(data.reply);
                chatHistory.push({ role: "model", text: data.reply });
                
                // Po każdej odpowiedzi wyświetlamy inteligentne podpowiedzi zamykające leada
                showSuggestions([
                ...
                console.warn("[Chatbot] Błąd API, przełączam na awaryjny tryb lokalny (regex):", data);
                processUserTextLocal(text);
            }
        })
        .catch(error => {
            console.error("[Chatbot] Błąd sieci, przełączam na awaryjny tryb lokalny:", error);
            removeTypingIndicator();
            processUserTextLocal(text);
        });
    }
    ...
    // Awaryjna, w 100% lokalna baza intencji (używana przy braku internetu / problemach z API)
    function processUserTextLocal(text) {
